...
while true; do
  case "$1" in
    ...
    -e|--enter) 
      shift
      case "$mode" in
        distrobox) distrobox enter "${1:-$arch_container_name}" -- "${2:-bash}" ;;
        proot-distro) proot-distro login "${1:-$arch_container_name}" --user $(cat $HOME/.archbox_user) --termux-home -- "${2:-sh -c bash}" ;;
      esac
      exit
    ;;
    -*)
